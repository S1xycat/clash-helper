...
$packageDir = Join-Path $releaseDir "ClashFloatStatus-v$version"
$zipPath = Join-Path $releaseDir "ClashFloatStatus-v$version.zip"
...
Copy-Item -LiteralPath (Join-Path $root "bin\ClashFloatStatus.exe") -Destination $packageDir
